...
function gallery(baseImages, options = {}) {
    ...
    this.createHtml = function () {
        ...
        if (options.showChevron) {
            html += `<div class="gallery-chevron-wrapper ${
                options.chevronsOnSide ? " on-side" : ""
            }"><svg class=gallery-chevron viewBox="0 0 24 24"><path d="M0 0h24v24H0V0z" fill="none" opacity=".87"/><path d="M17.51 3.87L15.73 2.1 5.84 12l9.9 9.9 1.77-1.77L9.38 12l8.13-8.13z"/></svg>
        ...
        this.chevrons.left = this.modal.querySelector(
            ".gallery-chevron:first-of-type",
        );
        this.chevrons.right = this.modal.querySelector(
            ".gallery-chevron:last-of-type",
        );
        ...
        if (options.slide) {
            ...
            this.galleryImages[galleryImages.length - 1].addEventListener(
                "load",
                () => {
                    let leftTotal = 0;
    ...
    this.moveImg = function (prevIndex, preventAnimation = false) {
        if (options.slide) {
            ...
            if (this.index > prevIndex && !preventAnimation) {
                ...
                if (this.slideIndex == this.galleryImages.length) {
                    ...
                    this.galleryImages[this.galleryImages.length - 1].after(
                    ...
                    this.galleryImages[
                        ...
                        parseInt(
                            this.galleryImages[this.galleryImages.length - 2]
                                .style.left,
                        ) +
            ...
            if (this.index < prevIndex && !preventAnimation) {
                ...
                if (this.slideIndex < 0) {
                    this.slideIndex = 0;
                    this.galleryImages[0].before(
                        this.galleryImages[this.galleryImages.length - 1],
                    );
                    this.galleryImages[0].style.left =
                        parseInt(this.galleryImages[1].style.left) -
                        this.galleryImages[0].offsetWidth +
                        "px";
                }
            }
            ...
            if (preventAnimation) {
                gotoIndex =
                    Array.from(wrapper.children).indexOf(
            ...
            // Real slide moving
            wrapper.style.left =
                parseInt(this.galleryImages[gotoIndex].style.left) * -1 + "px";
        ...
        if (options.showChevron && !preventAnimation) {
            ...
            if (
                (prevIndex < this.index && (prevIndex != 0 || index == 1)) ||
                (prevIndex == this.galleryImages.length - 1 && this.index == 0)
            ) {
        ...
        if (
            this.dummyImg.naturalHeight !=
                baseImages[this.index].naturalHeight ||
            this.dummyImg.naturalWidth != baseImages[this.index].naturalWidth
        ) {
            this.dummyImg.src = baseImages[this.index].src;
        }

        if (options.showName) {
            this.modal.querySelector(".gallery-img-name").textContent =
                baseImages[this.index].src.replaceAll(/.+\/|\..+/g, "");
        }
    ...
    this.setListeners = function () {
        ...
        this.modal
            .querySelectorAll(".gallery-chevron")
            .forEach((chevron, i) => {
        ...
        this.modal.addEventListener("click", (e) => {
            ...
            if (
                closeByClickAllowed &&
                (e.target.classList.contains("gallery-modal-full") ||
                    e.target.classList.contains("gallery-modal-dismiss"))
            ) {
                modal.classList.remove("gallery-show");
                modal.blur();
                return;
