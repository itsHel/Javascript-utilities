class Modal {
    static show(text, type = "", callback = false, once = false) {
        ...
        switch (type) {
            case "warn":
                ico =
                    '<svg viewBox="1 1 22 22"><path d="M0 0h24v24H0z" fill="none"/><circle cx="12" cy="19" r="2"/><path d="M10 3h4v12h-4z"/></svg>';
                heading =
                    "<div class='modal-heading warn-modal'><span class='modal-icons'>" +
                    ico +
                    "</span>Warning</div>";
                break;
            case "succ":
                ico =
                    '<svg viewBox="1 1 22 22"><path d="M0 0h24v24H0z" fill="none"/><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg>';
                heading =
                    "<div class='modal-heading succ-modal'><span class='modal-icons'>" +
                    ico +
                    "</span>Success</div>";
                break;
            case "notify":
                ...
                heading =
                    "<div class='modal-heading notify-modal'><span class='modal-icons'>" +
                    ico +
                    "</span>" +
                    type +
                    "</div>";
        ...
        if (!document.querySelector("#z-modal-full")) {
            ...
            modal.addEventListener("click", function (e) {
                if (
                    e.target.id == "z-modal-full" ||
                    e.target.id == "modal-dismiss" ||
            ...
            modal.addEventListener("transitionend", function (e) {
                if (
                    e.propertyName == "top" &&
                    modal.classList.contains("z-modal-show")
                ) {
            ...
            document.querySelector("#z-modal-full .modal-text").innerHTML =
                text;
            document.querySelector(
                "#z-modal-full .modal-heading-wrapper",
            ).innerHTML = heading;
        }
        ...
        if (callback) {
            ...
            confirm.closest(".modal-buttons").style.flexDirection =
                "row-reverse";
